...
-- 자격증 시험 뷰
CREATE VIEW vwCrtTestDetails AS
SELECT
    C.crtSeq,
    C.crtName,
    T1.crtCtg AS crtType,
...
-- 자격증 일정 뷰
CREATE VIEW vwCrtSchDetails AS 
SELECT
...
select 
    ...
    c.jobDesc
from tblcrt c 
    inner join tblcrttype ct 
        on c.crttypeseq = ct.crttypeseq
            inner join tblagency a
                on a.agencyseq = c.agencyseq
order by c.crtname;


select * from (select a.*, rownum as rnum from vwcrt a) where rnum between 1 and 10;

select * from tblcrt where crtname like '%정보처리기사%';

update tblcrt set difficulty=5  where crtseq = 80;

commit;

select * from tblcrt where difficulty='5';
select * from vwcrt where crtname like '%용접%' and difficulty=2;
select * from vwcrt where crtseq = 2177;
...
select * from vwCrtSchDday  where crtseq= 2177;
...
--select 
--    ctt.
--from tblCrtTestType ctt
--    inner join tblCrtTest ct
--        on ctt.crtTestTypeSeq = ct.crtTestTypeSeq
--            inner join vwcrt vc
--                on vc.crtSeq = ct.crtSeq;
...
select * from tblcrtboard where crtboardtitle like '%국정원%' or crtboardtitle like '%히어로즈%' or crtboardtitle like '%리그%' or crtboardtitle like '%월드오브%';
...
AS
SELECT
	boardseq, boardtitle, id, boardhits, boardlike, boardcontent,
	CASE
	...
	CASE
		...
		ELSE 0
	END AS isnew,
	(SELECT count(*) FROM tblcomment WHERE boardseq = tblBoard.boardseq) AS ccnt
