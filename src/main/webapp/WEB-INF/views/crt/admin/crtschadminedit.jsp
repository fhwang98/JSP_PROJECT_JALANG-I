...
#board-list > div > form > table > tbody > tr:nth-child(11) > td {
	width: 650px;
	height: 150px;
}

#board-list > div > form > table > tbody > tr:nth-child(12) > td {
	width: 650px;
	height: 150px;
}
...
<section class="notice">
  ...
  <!-- board list area -->
    <div id="board-list">
        <div class="container">
        	<form method="POST" action="/jr/crt/admin/crtschadminedit.do">
            <table class="board-table">
            
            	<!-- 자격증명(목록) 자격증시험명(목록), 회차, 시행기관(목록), 접수시작날짜, 접수종료날짜, 시험일, 합격자발표일을 입력한다. -->
				<tbody>        
				 <tr>
            		<th>자격증명 </th>
            		<td>${dto.crtName}</td>
         		...
         		<tr>
           			<th>자격증시험명 </th>
            		<td>${dto.crtTestName}</td>
         		</tr>
         		<tr>
            		<th>회차 </th>
            		<td><input type="text" name="round" required value="${dto.round}"></td>
         		</tr>
         		<tr>
            		<th>시행기관 </th>
            		<td>${dto.agency}</td>
         		...
         		<tr>
            		<th>접수시작날짜 </th>
            		<td><input type="text" name="testRcStartDate" required value="${dto.testRcStartDate}"></td>
         		</tr>
         		<tr>
            		<th>접수종료날짜 </th>
            		<td><input type="text" name="testRcEndDate" required value="${dto.testRcEndDate}"></td>
         		</tr>
         		<tr>
            		<th>시험시작일 </th>
            		<td><input type="text" name="testStartDate" required value="${dto.testStartDate}"></td>
         		</tr>
         		<tr>
            		<th>시험 종료일</th>
            		<td><input type="text" name="testEndDate" required value="${dto.testEndDate}"></td>
         		</tr>
         		<tr>
            		<th>합격자발표일 </th>
            		<td><input type="text" name="passDay" required value="${dto.passDay}"></td>
         		</tr>
				</tbody>
            </table>
			...
			<button class="button-79" role="button" onclick="location.href='/jr/crt/admin/crtschadmindetail.do?crtSchSeq=${dto.crtSchSeq}';">돌아가기</button>
			<input type="hidden" name="crtSchSeq" value="${dto.crtSchSeq}">
			</form>
			</div>
			</div>
        </div>
